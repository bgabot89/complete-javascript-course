...
// const bakeCake = function (ingredients, fn, date) {
//   const [first, ...rest] = ingredients.split(' ');
//   //Capitalize first letter and concotenate rest of string
//   const capitalizeFirst = first.slice(0, 1).toUpperCase() + first.slice(1);
//   console.log([capitalizeFirst, ...rest].join(''), dueDate(date));
// };

// bakeCake('cream, sugar, flour', dueDate, '6/10/24');
...
1. Based on the input number, update the answers array. For example, if the option is 3, 
...
changes the color of the selected h1 element ('header') to blue, each time the BODY element is clicked. 
...
(function () {
  ...
  //closure is necessary, IIFE, still able to access variables from its birthplace
